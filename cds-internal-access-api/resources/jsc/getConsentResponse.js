...
try {
    ...
    var acr = context.getVariable("OIDC.acr");
    context.setVariable('theACR', acr);
    var reqJwtUserInfo = context.getVariable("OIDC.user_info");
    if (reqJwtUserInfo) {
        reqJwtUserInfo = JSON.parse(reqJwtUserInfo);
    }
    var sharingDuration = context.getVariable("OIDC.sharing_duration");
    if (sharingDuration) {
        sharingDuration = parseInt(sharingDuration);
    }
    payload.consentRequested = {
        consentId: context.getVariable("cds.consentId"),
        scope: context.getVariable("OIDC.scope"),
        claims: {
            sharingDuration: sharingDuration,
            userinfo: reqJwtUserInfo,
            acr: ["urn:cds.au:cdr:2"]
        }
    };
    context.setVariable('responsePayload', JSON.stringify(payload));
    var arrangementHistory = context.getVariable("response.content");
    var requestedParCdrArrangementId = context.getVariable("OIDC.cdr_arrangement_id");
    if (arrangementHistory && requestedParCdrArrangementId !== "none") {
        arrangementHistory = JSON.parse(arrangementHistory);
    } 
    else {
        arrangementHistory = {};
    }

    payload.existingArrangement = arrangementHistory.arrangement;
    context.setVariable('responsePayload', JSON.stringify(payload));
